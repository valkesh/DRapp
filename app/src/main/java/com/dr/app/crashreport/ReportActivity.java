 ...
 */
package com.dr.app.crashreport;
...
public class ReportActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// TODO: Make use of a ProgressDialog while generating the
		// asynchronously

		/*
		 * // Note: Must be used to debug, because this activity starts // in a
		 * new process which is not being debugged before.
		 * if(!Debug.isDebuggerConnected()){ Debug.waitForDebugger(); }
		 */

		Throwable ex = (Throwable) getIntent().getSerializableExtra(
				EXTRA_THROWABLE);
		...
		target.putExtra(Intent.EXTRA_EMAIL,
				new String[] { ReportHandler.getEmailAddress() });
